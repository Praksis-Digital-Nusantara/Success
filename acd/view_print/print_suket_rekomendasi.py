...
from ..models import SuketRekomendasi, Pejabat
import textwrap



def print_suket_rekomendasi(request, id):
    context = web_name(request)

    data = SuketRekomendasi.objects.get(id=id)
    ...
    pos_y -= dl(p, A4[0] / 2, pos_y, 15, "SURAT REKOMENDASI", 'BU', 'C')
    ...
    pos_y -= dl(p, posd_x, pos_y, 30, "Memberikan rekomendasi kepada : ", 'N', 'L')
    ...
    pos_y = draw_aligned_text(p, "No. Telp", data.mhs.telp,  15)

    pos_y -= 15

    wrapped_text = textwrap.wrap(
        data.perihal + " sesuai syarat-syarat yang telah ditentukan dan telah dipenuhi oleh yang bersangkutan.", 
        width=470 
    )
    ...
    wrapped_text = textwrap.wrap(
        "Rekomendasi ini kami berikan " + data.alasan_rekomendasi, 
        width=470 
    )
    for line in wrapped_text: pos_y -= dl(p, posd_x, pos_y, 15, line, 'N', 'J', width=470)
    ...
    wrapped_text = textwrap.wrap(
        "Demikian surat keterangan ini kami berikan kepada yang bersangkutan untuk dipergunakan sebagaimana mestinya. ", 
        width=470 
    )
    for line in wrapped_text: pos_y -= dl(p, posd_x, pos_y, 15, line, 'N', 'J', width=470)
