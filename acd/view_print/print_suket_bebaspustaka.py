...
from ..models import SuketBebasPustaka, Pejabat
import textwrap



def print_suket_bebaspustaka(request, id):
    context = web_name(request)

    data = SuketBebasPustaka.objects.get(id=id)
    ...
    pos_y -= dl(p, A4[0] / 2, pos_y, 15, "SURAT KETERANGAN BEBAS PINJAM", 'BU', 'C')
    pos_y -= dl(p, A4[0] / 2, pos_y, 15, "Nomor: " + data.no_surat, 'N', 'C')

    pos_y -= dl(p, posd_x, pos_y, 30, "Yang bertanda tangan dibawah ini, Pengelola Perpustakaan Fakultas Ekonomi Universitas Negeri Makassar menerangkan bahwa:", 'N', 'L')
    ...
    pos_y -= dl(p, posd_x, pos_y, 30, "Benar saudara tersebut di atas telah bebas dari pinjaman dan hal-hal yang berkaitan dengan Perpustakaan Fakultas Ekonomi Universitas Negeri Makassar:", 'N', 'L')
    wrapped_text = textwrap.wrap(
        "Demikian surat keterangan ini diberikan kepadanya untuk digunakan sebagaimana mestinya. ", 
        width=470 
    )
    ...
    if data.ttd and data.ttd.pejabat:
        ...
        pos_y -= dl(p, pos_x_ttd, pos_y, 15, "Pustakawan FE UNM", 'N', 'L')
        # pos_y -= dl(p, pos_x_ttd, pos_y, 15, "Universitas Negeri Makassar", 'N', 'L')
        if data.ttd_status == 'QRcode' :
            p.drawImage(ImageReader(context.get("api_qrcode", "") + context.get("baseurl", "") + 't/sbpe/' + str(data.id)), pos_x_ttd+10, pos_y-50, width=40, height=40)
