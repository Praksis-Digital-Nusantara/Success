...
from ..models import SuketBerkelakuanBaik, Pejabat
import textwrap



def print_suket_berkelakuanbaik(request, id):
    context = web_name(request)

    data = SuketBerkelakuanBaik.objects.get(id=id)
    ...
    pos_y -= dl(p, A4[0] / 2, pos_y, 15, "SURAT KETERANGAN BERKELAKUAN BAIK", 'BU', 'C')
    pos_y -= dl(p, A4[0] / 2, pos_y, 15, "Nomor: " + data.no_surat, 'N', 'C')

    pos_y -= dl(p, posd_x, pos_y, 30, "Wakil Dekan Bidang Kemahasiswaan dan Alumni Fakultas Ekonomi dan Bisnis Universitas Negeri Makassar dengan ini menerangkan bahwa:", 'N', 'L')
    ...
    pos_y = draw_aligned_text(p, "No. HP", data.mhs.telp,  15)

    pos_y -= dl(p, posd_x, pos_y, 30, "Sepanjang pengetahuan kami, yang bersangkutan adalah:", 'N', 'L')

    pos_y -= dl(p, posd_x, pos_y, 30, "A. Berkelakuan baik selama menjadi mahasiswa", 'N', 'L')
    pos_y -= dl(p, posd_x, pos_y, 15, "B. Tidak mempunyai catatan kriminal dari pihak kepolisian", 'N', 'L')
    pos_y -= dl(p, posd_x, pos_y, 15, "C. Tidak terlibat aksi unjuk rasa yang menentang kebijakan pemerintah yang dapat merusak nama baik pribadi maupun Universitas Negeri Makassar", 'N', 'L')
    ...
    if data.ttd and data.ttd.pejabat:
        ...
        pos_y -= dl(p, pos_x_ttd, pos_y, 15, "Wakil Dekan Bidang Kemahasiswaan dan Alumni", 'N', 'L')
        ...
        if data.ttd_status == 'QRcode' :
            p.drawImage(ImageReader(context.get("api_qrcode", "") + context.get("baseurl", "") + 't/skb/' + str(data.id)), pos_x_ttd+10, pos_y-50, width=40, height=40)
