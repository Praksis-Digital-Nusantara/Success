...
def print_suket_bebaskuliah(request, id):
    context = web_name(request)

    data = SuketBebasKuliah.objects.get(id=id)
    ...
    pos_y -= dl(p, A4[0] / 2, pos_y, 15, "SURAT KETERANGAN BEBAS BEBAN KULIAH", 'B', 'C')
    ...
    wrapped_text = textwrap.wrap(
        "adalah benar mahasiswa program studi " + data.mhs.prodi.nama_prodi + " " +  context.get("faculty_name", "") + " " + context.get("university_name", "") + " yang telah lulus dalam mata kuliah dengan jumlah " + str(data.sks_lulus) + " sks. Sehingga mahasiswa tersebut dinyatakan bebas terhadap beban kuliah.",
        width=100 
    )
    ...
    if data.ttd_status == 'QRcode' :
        p.drawImage(ImageReader(context.get("api_qrcode", "") + context.get("baseurl", "") + 't/sbk/' + str(data.id)), pos_x_ttd+10, pos_y-50, width=40, height=40)
    ...
    p.setTitle("Bebas Kuliah ")
    p.showPage()
    p.save()
    response["Content-Disposition"] = f'inline; filename="Bebas Kuliah.pdf"'
    return response
